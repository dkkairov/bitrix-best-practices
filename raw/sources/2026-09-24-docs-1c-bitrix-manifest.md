...
|---|---|---|
| `modules` | 48 | нет |
| `framework` | 37 (≈22 уникальных: часть страниц продублирована транслитерированными адресами) | **частично, 2026-09-24** — взяты роутинг, контроллеры, очереди сообщений; в очереди: расширения, пре- и постфильтры, конфигурация ядра, cookie, хранилище, сессии, компоненты, консольные команды + пересверка наших страниц про события, приложение, ServiceLocator, Result |
| `ui` | 34 | нет |
| `get-started` | 20 | нет |
| `cms-basics` | 16 | нет |
