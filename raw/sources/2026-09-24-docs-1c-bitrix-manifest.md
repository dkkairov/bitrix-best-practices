...
|---|---|---|
| `modules` | 48 | **частично, 2026-09-24** — взяты highload-блоки (8 страниц) и инфоблоки (4) → [[concept-highload-blocks]], [[concept-iblocks-and-lists]]; **`catalog` (10) и `sale` (25) осознанно отложены**: это «1С-Битрикс: Управление сайтом», вне фокуса вики по внедрению Bitrix24 (`CLAUDE.md` §1). Вернёмся, если появится проект с интернет-магазином |
...
| `ui` | 34 | **да, 2026-09-24** — четыре страницы: селектор сущностей, боковая панель, попапы и меню, системные компоненты (ui.system.*, иконки, a11y); фильтр и грид у нас уже были, `vue` и `lottie` отложены |
| `get-started` | 20 (≈11 уникальных) | **сверка, 2026-09-24** — «Как безопасно кастомизировать» и «Composer» вплетены в [[concept-change-invasiveness-hierarchy]] и [[recipe-composer-third-party-libraries]]; создание модуля, компонента, контроллера и структура каталогов у нас уже описаны, установка окружения — свой рецепт стенда |
| `cms-basics` | 16 | **частично, 2026-09-24** — взяты почта, пользовательские поля, пользователи и группы; шаблоны сайта, меню, включаемые области, хлебные крошки, стили, заголовки, постраничная навигация относятся к публичной части сайта и отложены |
...
| `about` | 3 | **прочитано, страниц не требуется** — сравнения продуктов; технические тезисы (контроллеры, ActionFilter, агенты, ORM, почтовые события) уже описаны у нас |
