...
- [[recipe-console-commands|Консольные команды ядра: bitrix.php и свои команды]]
...
- [[entity-http-request-response|HttpRequest, HttpResponse и Context]] — запрос и ответ
...
Из раздела «Основы фреймворка» взяты роутинг, контроллеры, очереди сообщений, конфигурация ядра,
сессии и хранилище, cookie, запрос и ответ, консольные команды; сверены страницы про события,
`ServiceLocator` и `Result`. Осталось: архитектура, автозагрузка, фоновые задачи, пре- и
постфильтры, жизненный цикл запроса, валидация — там нужна пересверка наших страниц.
