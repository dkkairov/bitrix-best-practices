...
status: draft
provenance: mixed
verified: "2026-09-21 / «Книга разработчика Bitrix24» (bx24devbook, снимок 2026-09-21): Общие сведения / Выполнение страницы (порядок выполнения, фоновый режим); Технологии / Агенты; Структура папки local; запуск от владельца сайта — эмпирика вики; без проверки на стенде"
...
- **нет авторизованного пользователя** — всё выполняется от гостя;
- **нет сессии и cookie** — состояние между запусками так не сохранить;
- **`$_SERVER` пуст** — его заполняет веб-сервер, а не `php -f`. На `SERVER_NAME` и соседей не
  опираемся; `DOCUMENT_ROOT` задаём сами — абсолютный путь к каталогу, где лежит `bitrix`.
...
4. **Лимиты — после пролога:** `set_time_limit(0)` и `ignore_user_abort(true)`. Раньше нельзя —
   `dbconn.php` может их переопределить.
5. **Полезный код.**
...
// 6. Завершение: закрыть БД, выполнить фоновые задания
\CMain::FinalActions();
```
...
- В `bitrix/managed_cache` и `bitrix/cache` нет файлов чужого владельца — команда проверки в
  [[antipattern-cli-php-as-root]].
- **Проверить на стенде:** выполняется ли код после `FinalActions()`. От этого зависит, где ставить
  `exit(<код>)`, если мониторинг cron смотрит на код возврата.

## Чего избегать

- ❌ Запуска от root — см. «Запуск».
- ❌ `set_time_limit` / `ignore_user_abort` до пролога.
- ❌ Скрипта без `FinalActions()` или служебного эпилога.
