...
- **Сроки и ожидания** (прогон рецептов, [[recipe-bizproc-approval-route]],
  [[recipe-bizproc-request-intake]]). По истечении срока «Утверждение» отклоняется само: в журнале
  «Документ отклонен» без голосовавшего, `IsTimeout` = 1, ветка «нет». «Запрос дополнительной
  информации» закрывается, процесс идёт дальше с пустыми ответами. `CommentRequired: YR` — отказ без
  пояснения не принимается («Не заполнено поле: Пояснение»). Сроки снимает агент — в пределах двух
  минут на агентах cron. «Поставить задачу» с ожиданием продолжает процесс сразу после завершения
  задачи исполнителем, при удалении — «Задача была удалена» и `IsDeleted` = 1.
