...
status: draft
provenance: mixed
verified: "2026-09-21 / «Книга разработчика Bitrix24» (bx24devbook, снимок 2026-09-21): Модуль CRM / Лид, Контакт, Компания, Сделка — Описание, Методы, Примеры; Универсальное API / Концепция, Как включить; версия модуля crm в книге не указана — перепроверять при каждом обновлении crm; без проверки на стенде"
...
> **Черновик со сроком годности.** Для этих четырёх сущностей Universal API пока необязателен.
> Книга называет его поддержку экспериментальной: позже UA включат принудительно, а старое поведение
> уберут ([Концепция UA](https://bx24devbook.website.yandexcloud.net/Modul_CRM/Universalnoe_api/Koncepcia.html#novoe-universal-noe-api)).
> Версию модуля `crm` книга не называет, в её примерах есть синтаксические ошибки, живой проверки не
> было. Перепроверять при каждом обновлении `crm`.

## Когда применять

- Для сущности **выключен** Universal API (проверка — в предусловиях). Если включён, пишите через
...
- Ключи фильтра `__JOINS` (список массивов с `TYPE` и `SQL`) и `__CONDITIONS` (список массивов с
  ...
  Псевдонимы основной таблицы по книге: лид — `L`, компания — `CO`, сделка — `D`. Для контакта книга
  противоречит себе (в комментарии `C`, в коде `L`) — **проверить на стенде**.
...
- **`LEAD_ID`** у контакта, компании и сделки в таблицах полей помечен `RO`, а глава о конвертации
  велит проставлять его через старое API. Книга противоречит себе — **проверить на стенде**; подробнее
  в [[recipe-crm-lead-conversion]].
...
Для лида книга берёт тот же `ContactDuplicateChecker`, для компании пишет имя класса иначе. Какой
проверяльщик нужен лиду и компании — **проверить на стенде**. Чтение мультиполей элемента —
