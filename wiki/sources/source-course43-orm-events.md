...
verified: "2026-09-23 / dev.1c-bitrix.ru, COURSE_ID=43: 26 уроков раздела ORM (в два захода) и 2 урока раздела «События» (снимок-манифест raw/sources/2026-09-23-course43-orm-events-manifest.md); спорное по первой пачке сверено на коробке main 26.750.0, уроки о связях — пока только по курсу"
...
уровень Middle, разделы **ORM** (26 уроков) и **События** (2 урока). Снимок-манифест с датами и
...
**Связи (уроки 11737, 11739, 11741, 11707)**
- Внешний ключ связью не является: связь описывается полем `Reference` (много-к-одному и
  один-к-одному), `OneToMany` (обратная сторона) или `ManyToMany` (через промежуточную таблицу).
- В условии join префиксы `this.` и `ref.`; помощник `Join::on()` — это фильтр с готовым
  `whereColumn`, к которому можно дописать любые условия. Тип join по умолчанию `left`.
- В `select` связь указывают явно (`'*'` её не тянет); в массивах колонки получают длинные имена,
  лечится алиасом. Объекты не двоят данные — партнёры склеиваются в коллекцию.
- Менять связи только методами `addTo*`, `removeFrom*`, `removeAll*` и затем `save()`; прямые
  операции над коллекцией-значением не работают. Удаляется связь, а не записи.
- Если у связи есть свои данные (количество, статус) — `ManyToMany` не подходит: промежуточную
  таблицу описывают отдельной сущностью и двумя `OneToMany`.

**Заполнение, восстановление, аннотации (уроки 11705, 11703, 11733, 11715, 11755)**
- `fill()` — правильный способ дочитать поля; ручное «прочитать и `set()`» делает значение
  изменённым, а не актуальным. Маски: `SCALAR`, `EXPRESSION`, `USERTYPE`, `REFERENCE`,
  `ONE_TO_MANY`, `MANY_TO_MANY`, `FLAT`, `RELATION`, `ALL`. В цикле по объектам `fill()` не зовут —
  только у коллекции.
- `wakeUp()` принимает вложенные значения связей.
- Объект поддерживает `ArrayAccess` (`$author['NAME']`), но runtime-поля только на чтение.
- Аннотации для IDE: ядро — `/bitrix/modules/main/meta/orm.php` (с `main` 20.100.0), свои модули —
  CLI `php bitrix.php orm:annotate -m <модуль>`.
- С `main` 18.0.4: имена полей регистронезависимы, алиас в другом регистре запрещён, `BooleanField`
  не принимает пустую строку, **сериализованные поля в объектах не поддерживаются**.
...
| [[concept-d7-orm-relations]] | новая (черновик): `Reference`, `OneToMany`, `ManyToMany`, аннотации, совместимость |
...
- [[concept-d7-orm-relations]] написана по курсу, но **не прогнана на стенде** (был недоступен
  Docker): список проверок — в конце той страницы.
