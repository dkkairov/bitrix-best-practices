...
| 6 | [docs.1c-bitrix.ru — документация Bitrix Framework](https://docs.1c-bitrix.ru/) | каталог-указатель + содержание; **эталон** (`CLAUDE.md` §9), новый формат официальной документации | [development/](../development/core-d7/_index-core-d7.md) | **P1** | 221 страница, снимок-манифест `raw/sources/2026-09-24-docs-1c-bitrix-manifest.md`. Взято: `orm` (сверка), **`performance` (6), `database` (12), `advanced` (12 из 16) — 2026-09-24**; в `advanced` отложены `uuid`, `encoding`, `geolocation` (по требованию) и `vue` (с разделом `ui`); из `framework` взяты роутинг, контроллеры, очереди сообщений. Очередь: дочитать `framework` (расширения, пре/постфильтры, конфигурация ядра, cookie, хранилище, сессии, компоненты, консольные команды + пересверка наших страниц про события, приложение, ServiceLocator, `Result`) → `security` (14) → `ui` (34) → `modules` (48) → `cms-basics` (16) → `get-started` (20) |
