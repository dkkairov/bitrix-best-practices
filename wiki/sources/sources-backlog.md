...
| 6 | [docs.1c-bitrix.ru — документация Bitrix Framework](https://docs.1c-bitrix.ru/) | каталог-указатель + содержание; **эталон** (`CLAUDE.md` §9), новый формат официальной документации | [development/](../development/core-d7/_index-core-d7.md) | **P1** | 221 страница, снимок-манифест `raw/sources/2026-09-24-docs-1c-bitrix-manifest.md`. Взято: `orm` (сверка), **`performance` (6), `database` (12), `advanced` (12 из 16) — 2026-09-24**; в `advanced` отложены `uuid`, `encoding`, `geolocation` (по требованию) и `vue` (с разделом `ui`); **`framework`, `security`, `ui` пройдены — 2026-09-24**; из `modules` взяты highload-блоки и инфоблоки, а `catalog` (10) и `sale` (25) отложены как тема «1С-Битрикс: Управление сайтом» — вне фокуса вики. из `cms-basics` взяты почта, UF, пользователи и группы; `get-started` и `about` сверены. **Пройдено всё, что относится к внедрению Bitrix24.** Осталось по требованию: `catalog` и `sale` (интернет-магазин), публичная часть сайта из `cms-basics` (меню, шаблоны, хлебные крошки, стили), `uuid`/`encoding`/`geolocation`/`vue` из `advanced`, `modules/architecture` |
